# Navegador Plongê — Arquitetura Técnica da Solução

Documentos relacionados: [`produto_funcionalidades.md`](produto_funcionalidades.md) (PRD), [`mapa_de_telas.md`](mapa_de_telas.md) (UI), [`modelagem.md`](modelagem.md) (dados).

## Glossário breve

| Termo | Significado |
|--------|-------------|
| **Navegador** / **Navegador Plongê** | Produto (frontend Next.js + API). |
| **Plia** | Camada de IA (LLM, RAG, serviços `LlmQueryService`, embeddings). |
| **Plia v1** | Legado anterior; não confundir com este stack. |
| **Superfície Plia** | UI onde se usa a Plia; **composer** (chat/busca) fixo na **parte inferior** da tela. |
| **Locale** | Idioma/região ativos para UI e mensagens de sistema (`pt-BR`, `en-US`, …). |

---

## 1. Visão Geral

O **Navegador Plongê** é a evolução da plataforma da Plongê, desenhada para transformar conhecimento operacional disperso (entrevistas, conversas com clientes, CVs de candidatos) em inteligência consultável e proprietária da empresa.
...
- **Frontend (Next.js):** catálogos de strings por locale (ex. **next-intl** ou equivalente); rotas com segmento `[locale]` ou locale em cookie + middleware; **pt-BR** como fallback padrão.
- **API:** textos gerados pelo servidor (validações GraphQL, emails Sidekiq, rótulos em exports) parametrizados por **locale**; preferência persistida (ex. `users.locale`).
- **Dados:** texto livre registado pelos utilizadores **não** exige tradução no núcleo; labels de sistema e workflows configuráveis podem evoluir para tabelas de tradução.
- **Plia:** alinhar respostas ao locale da sessão quando o modelo permitir.

### 2.2 AppShell (produto)

O cliente Next.js segue o desenho de produto: navegação de módulos à **esquerda** (em mobile, **barra inferior**), **conteúdo rico** ao centro, **painel de detalhes** à direita (drawer em mobile). A **Superfície Plia** apresenta o **composer** sempre na **borda inferior** da viewport. Detalhe: [`produto_funcionalidades.md`](produto_funcionalidades.md) §4.
...
## 5. Camada Plia (IA) — RAG e consultas

Esta secção descreve os serviços de **Plia** no backend (`LlmQueryService`, `EmbeddingService`, `ConfidentialDataMasker` no diagrama acima).
...
## 9. Diferenciais Estratégicos do Navegador Plongê
...
## 11. Fluxos ponta-a-ponta (RBAC, jobs, extensões)

### 11.1 Leitura de dados com escopo (qualquer tela de lista/detalhe)

1. Utilizador autenticado (`JWT`/sessão conforme implementação).
2. Resolver `current_user` e **conjunto de contas/projetos** autorizados (`user_roles`).
3. Queries GraphQL aplicam **filtro obrigatório** por `company_id` / `project_id` — nunca confiar só no cliente.
4. Resposta renderizada no AppShell (lista → inspector).

### 11.2 Upload de documento → embeddings

1. Upload direto ou via API → **Active Storage** (S3).
2. Registo em `documents` com `documentable` e metadados.
3. **GenerateEmbeddingsJob** (Sidekiq): chunking → API de embeddings → `document_chunks`.
4. Consultas na **Superfície Plia**: retrieval filtrado pelo mesmo escopo RBAC (§5.4).

### 11.3 Consulta Plia (composer inferior)

1. Texto da pergunta + contexto de escopo (cliente/projeto/datas) enviados ao resolver.
2. **Classificação** da query (estruturada / sumarização / sensível — §5.1).
3. SQL ou RAG conforme tipo; **ConfidentialDataMasker** antes de LLM externo quando aplicável.
4. **query_audit_logs** grava pedido, escopo e contagens.
5. Resposta devolvida ao cliente; a UI mostra mensagens **acima** do composer fixo.

### 11.4 MCP / automação (evolução)

O produto prevê **MCP** para acções estruturadas (registar conversa a partir de texto, etc.). Na implementação, expor ferramentas com **mesmo RBAC** que resolvers GraphQL e registos de auditoria por mutação sensível.

### 11.5 Integrações externas (Lusha, WhatsApp, extensão LinkedIn)

- **Lusha / WhatsApp:** camada de integração na API (REST ou webhooks futuros); não bypass de RBAC.
- **Extensão navegador:** fluxo autenticado para associar perfil a **projeto/candidato**; tokens com escopo limitado; rate limit.

### 11.6 ATS interno e integrações com ATS de clientes (evolução)

- O Navegador **é** o **ATS operacional da Plongê** (projetos, candidatos, etapas, documentos) — ver [`modelagem.md`](modelagem.md) e [`produto_funcionalidades.md`](produto_funcionalidades.md).
- **v2+:** conectores opcionais para ATS **dos clientes** (ex.: **Gupy**, **Greenhouse**, **SAP SuccessFactors**):
  - Camada de **adapters** na API (jobs dedicados Sidekiq, filas por tenant/cliente).
  - Credenciais e tokens por **conta** (cliente da Plongê); auditoria de sincronização.
  - Respeitar limites de API e políticas de dados (muitas vezes o candidato é titular no sistema do cliente — alinhar contratos e bases legais antes de sync bidirecional).

---

*Documento atualizado em maio de 2026. Inclui navegação documental e fluxos; stack inalterada na essência.*
